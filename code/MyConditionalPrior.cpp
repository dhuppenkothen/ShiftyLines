...
double MyConditionalPrior::log_pdf(const std::vector<double>& vec) const
{
	...
	if(dshift < -0.01 || dshift > 0.01)
		return -1E300;

	double logprior = 0.;

//	logprior += -log(1.E-3) - log(1. + pow(dshift, 2)/pow(1.E-3, 2));
...

{
	...
	vec[0] = -0.01 + (0.01 - (-0.01))*vec[0]; // Doppler shift
//	vec[0] = 1E-3*tan(M_PI*(vec[0] - 0.5));
...
void MyConditionalPrior::to_uniform(std::vector<double>& vec) const
{
	...
	vec[0] = (vec[0] - (-0.01))/(0.01 - (-0.01));
//	vec[0] = (1./M_PI)*atan(vec[0]/1E-3) + 0.5;
